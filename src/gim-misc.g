...
{
  gim_biz_error(client, ERROR_SUCCESS, ERRMSG_SUCCESS);
}
              ...
              json_object* payload)
{
  ...
  if (member_id[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_MEMBER_ID, ERRMSG_PARAM_NO_MEMBER_ID);
    return;
  }
  if (member_type[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_MEMBER_TYPE, ERRMSG_PARAM_NO_MEMBER_TYPE);
    return;
  }
               ...
               json_object* payload)
{
  ...
  gim_util_get_string(payload, "sessionId", session_id);

  if (session_id[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_SESSION_ID, ERRMSG_PARAM_NO_SESSION_ID);
    return;
  }
                                ...
                                json_object* payload)
{
  ...
  {
    gim_biz_error(client, ERROR_PARAM_NO_CONVERSATION_ID, ERRMSG_PARAM_NO_CONVERSATION_ID);
    return;
  }
  if (member_id[0] == '\0')
  {
                     ...
                     json_object* payload)
{
  ...
  gim_util_get_string(payload, "conversationId", conversation_id);
  gim_util_get_string(payload, "senderId", sender_id);
  gim_util_get_string(payload, "senderType", sender_type);
  ...
  if (sender_id[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_SENDER_ID, ERRMSG_PARAM_NO_SENDER_ID);
    return;
  }
  if (sender_type[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_SENDER_TYPE, ERRMSG_PARAM_NO_SENDER_TYPE);
    return;
  }
  if (message_type[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_MESSAGE_TYPE, ERRMSG_PARAM_NO_MESSAGE_TYPE);
    return;
  }
  if (message_content[0] == '\0')
  {
    gim_biz_error(client, ERROR_PARAM_NO_MESSAGE_CONTENT, ERRMSG_PARAM_NO_MESSAGE_CONTENT);
    return;
  }
                     ...
                     json_object* payload)
{
  ...
  gim_util_get_string(payload, "memberId", member_id);
  gim_util_get_string(payload, "memberType", member_type);
  gim_util_get_string(payload, "lastConversationTime", last_conversation_time);
  ...
  {
    gim_biz_error(client, ERROR_PARAM_NO_LAST_CONVERSATION_TIME, ERRMSG_PARAM_NO_LAST_CONVERSATION_TIME);
    return;
  }
              ...
              json_object* payload)
{
  ...
  {
    gim_biz_error(client, ERROR_UNAUTHORIZED_USER, ERRMSG_UNAUTHORIZED_USER);
    return;
  }
...
*/
void
gim_biz_send(ws_cli_conn_t* client,
             json_object* payload)
{
  // 用户验证
  json_object *jval = json_object_object_get(payload, "userId");
                          ...
                          json_object* payload)
{
  ...
  gim_util_get_string(payload, "memberId", member_id);
  gim_util_get_string(payload, "memberType", member_id);
